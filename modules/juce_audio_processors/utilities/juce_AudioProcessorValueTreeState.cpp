...
AudioProcessorValueTreeState::Parameter::Parameter(
    const ParameterID &parameterID, const String &parameterName,
    NormalisableRange<float> valueRange, float defaultParameterValue,
    const AudioProcessorValueTreeStateParameterAttributes &attributes)
    : AudioParameterFloat(parameterID, parameterName, valueRange,
                          defaultParameterValue,
                          attributes.getAudioParameterFloatAttributes()),
      unsnappedDefault(valueRange.convertTo0to1(defaultParameterValue)),
      discrete(attributes.getDiscrete()), boolean(attributes.getBoolean()) {}
...
public:
  explicit ParameterAdapter(RangedAudioParameter &parameterIn)
      : parameter(parameterIn),
        // For legacy reasons, the unnormalised value should *not* be snapped on
        // construction
        unnormalisedValue(
            getRange().convertFrom0to1(parameter.getDefaultValue())) {
...
private:
  ...
  void parameterValueChanged(int, float) override {
    ...
    listeners.call([this](Listener &l) {
      l.parameterChanged(parameter.paramID, unnormalisedValue);
    });
  ...
  public:
    template <typename Fn> void call(Fn &&fn) {
...
AudioProcessorValueTreeState::AudioProcessorValueTreeState(
    AudioProcessor &processorToConnectTo, UndoManager *undoManagerToUse,
  ...
  struct PushBackVisitor : ParameterLayout::Visitor {
    ...
    void
    visit(std::unique_ptr<AudioProcessorParameterGroup> group) const override {
      if (group == nullptr) {
        jassertfalse;
        return;
      }
      ...
      for (const auto param : group->getParameters(true)) {
        if (const auto rangedParam =
                dynamic_cast<RangedAudioParameter *>(param)) {
          state->addParameterAdapter(*rangedParam);
        } else {
          // If you hit this assertion then you are attempting to add a
          // parameter that is not derived from RangedAudioParameter to the
...
AudioProcessorValueTreeState::AudioProcessorValueTreeState(AudioProcessor &p,
                                                           UndoManager *um)
...
AudioProcessorValueTreeState::~AudioProcessorValueTreeState() { stopTimer(); }

//==============================================================================
RangedAudioParameter *AudioProcessorValueTreeState::createAndAddParameter(
    const String &paramID, const String &paramName, const String &labelText,
    NormalisableRange<float> range, float defaultVal,
    std::function<String(float)> valueToTextFunction,
    std::function<float(const String &)> textToValueFunction,
    bool isMetaParameter, bool isAutomatableParameter, bool isDiscreteParameter,
    AudioProcessorParameter::Category category, bool isBooleanParameter) {
  auto attributes =
      AudioProcessorValueTreeStateParameterAttributes()
          .withLabel(labelText)
          .withStringFromValueFunction([fn = std::move(valueToTextFunction)](
                                           float v, int) { return fn(v); })
  ...
  return createAndAddParameter(std::make_unique<Parameter>(
      paramID, paramName, range, defaultVal, std::move(attributes)));
}
...
RangedAudioParameter *AudioProcessorValueTreeState::createAndAddParameter(
  ...
  // All parameters must be created before giving this manager a ValueTree
  // state!
...
//==============================================================================
void AudioProcessorValueTreeState::addParameterAdapter(
    RangedAudioParameter &param) {
  adapterTable.emplace(param.paramID,
                       std::make_unique<ParameterAdapter>(param));
}

AudioProcessorValueTreeState::ParameterAdapter *
AudioProcessorValueTreeState::getParameterAdapter(StringRef paramID) const {
  auto it = adapterTable.find(paramID);
  return it == adapterTable.end() ? nullptr : it->second.get();
}

void AudioProcessorValueTreeState::addParameterListener(StringRef paramID,
                                                        Listener *listener) {
  if (auto *p = getParameterAdapter(paramID))
    p->addListener(listener);
}

void AudioProcessorValueTreeState::removeParameterListener(StringRef paramID,
                                                           Listener *listener) {
  if (auto *p = getParameterAdapter(paramID))
    p->removeListener(listener);
}

Value AudioProcessorValueTreeState::getParameterAsValue(
    StringRef paramID) const {
...
NormalisableRange<float> AudioProcessorValueTreeState::getParameterRange(
    StringRef paramID) const noexcept {
  if (auto *p = getParameterAdapter(paramID))
    return p->getRange();
...
AudioProcessorValueTreeState::getParameter(StringRef paramID) const noexcept {
  if (auto adapter = getParameterAdapter(paramID))
...
std::atomic<float> *AudioProcessorValueTreeState::getRawParameterValue(
    StringRef paramID) const noexcept {
  if (auto *p = getParameterAdapter(paramID))
...
void AudioProcessorValueTreeState::setNewState(ValueTree vt) {
  ...
  if (auto *p = getParameterAdapter(vt.getProperty(idPropertyID).toString())) {
    p->tree = vt;
    p->setDenormalisedValue(
        p->tree.getProperty(valuePropertyID, p->getDenormalisedDefaultValue()));
  }
}
...
void AudioProcessorValueTreeState::updateParameterConnectionsToChildTrees() {
  ...
  for (auto &p : adapterTable) {
    ...
    if (!adapter.tree.isValid()) {
      ...
      adapter.tree.setProperty(idPropertyID, adapter.getParameter().paramID,
                               nullptr);
...
void AudioProcessorValueTreeState::valueTreePropertyChanged(
    ValueTree &tree, const Identifier &) {
  if (tree.hasType(valueType) && tree.getParent() == state)
    setNewState(tree);
}

void AudioProcessorValueTreeState::valueTreeChildAdded(ValueTree &parent,
                                                       ValueTree &tree) {
...
makeAttachment(const AudioProcessorValueTreeState &stateToUse,
               const String &parameterID, Control &control) {
  if (auto *parameter = stateToUse.getParameter(parameterID))
    return std::make_unique<Attachment>(*parameter, control,
...
AudioProcessorValueTreeState::SliderAttachment::SliderAttachment(
    ...
    Slider &slider)
    : attachment(makeAttachment<SliderParameterAttachment>(
          stateToUse, parameterID, slider)) {}

AudioProcessorValueTreeState::ComboBoxAttachment::ComboBoxAttachment(
    AudioProcessorValueTreeState &stateToUse, const String &parameterID,
    ComboBox &combo)
    : attachment(makeAttachment<ComboBoxParameterAttachment>(
          stateToUse, parameterID, combo)) {}

AudioProcessorValueTreeState::ButtonAttachment::ButtonAttachment(
    AudioProcessorValueTreeState &stateToUse, const String &parameterID,
    Button &button)
    : attachment(makeAttachment<ButtonParameterAttachment>(
          stateToUse, parameterID, button)) {}
...
struct ParameterAdapterTests : public UnitTest {
  ParameterAdapterTests()
      : UnitTest("Parameter Adapter",
                 UnitTestCategories::audioProcessorParameters) {}
  ...
  void runTest() override {
    ...
    {
      const auto test = [&](NormalisableRange<float> range, String text,
                            float expected) {
        AudioParameterFloat param({}, {}, range, {});
        AudioProcessorValueTreeState::ParameterAdapter adapter(param);
...
public:
  AudioProcessorValueTreeStateTests()
      : UnitTest("Audio Processor Value Tree State",
                 UnitTestCategories::audioProcessorParameters) {}
  ...
  void runTest() override {
    ...
    beginTest("After calling createAndAddParameter, the number of parameters "
              "increases by one");
    {
      TestAudioProcessor proc;

      proc.state.createAndAddParameter(std::make_unique<Parameter>(
          String(), String(), NormalisableRange<float>(), 0.0f));

      expectEquals(proc.getParameters().size(), 1);
    }

    beginTest("After creating a normal named parameter, we can later retrieve "
              "that parameter");
    {
      TestAudioProcessor proc;
      ...
      const auto param =
          proc.state.createAndAddParameter(std::make_unique<Parameter>(
              key, String(), NormalisableRange<float>(), 0.0f));
      ...
      TestAudioProcessor proc(
          {std::make_unique<AudioProcessorParameterGroup>(
               ...
               std::make_unique<AudioParameterFloat>(
                   "b", "", NormalisableRange<float>{}, 0.0f)),
           std::make_unique<AudioProcessorParameterGroup>(
               "B", "", "",
               std::make_unique<AudioParameterInt>("c", "", 0, 1, 0),
               std::make_unique<AudioParameterChoice>(
                   "d", "", StringArray{"foo", "bar"}, 0))});
    ...
    beginTest("Meta parameters can be created");
    {
      ...
      const auto key = "id";
      const auto param = proc.state.createAndAddParameter(
          std::make_unique<Parameter>(key, String(), NormalisableRange<float>(),
                                      0.0f, Attributes().withMeta(true)));
    ...
    beginTest("Discrete parameters can be created");
    {
      ...
      const auto param = proc.state.createAndAddParameter(
          std::make_unique<Parameter>(key, String(), NormalisableRange<float>(),
                                      0.0f, Attributes().withDiscrete(true)));
    ...
    beginTest("Boolean parameters can be created");
    {
      ...
      const auto param = proc.state.createAndAddParameter(
          std::make_unique<Parameter>(key, String(), NormalisableRange<float>(),
                                      0.0f, Attributes().withBoolean(true)));

      expect(param->isBoolean());
    }

    beginTest("After creating a custom named parameter, we can later retrieve "
              "that parameter");
    {
      const auto key = "id";
    ...
    beginTest("After adding a normal parameter that already exists, the "
              "AudioProcessor parameters are unchanged");
    {
      TestAudioProcessor proc;
      const auto key = "id";
      const auto param =
    ...
    beginTest("After setting a parameter value, that value is reflected in the "
              "state");
    {
      TestAudioProcessor proc;
      const auto key = "id";
        ...
        "After adding an APVTS::Parameter, its value is the default value");
    {
      ...
      proc.state.createAndAddParameter(std::make_unique<Parameter>(
          key, String(), NormalisableRange<float>(0.0f, 100.0f, 10.0f), value));
    ...
    beginTest("Bool parameters have a range of 0-1");
    {
      ...
      TestAudioProcessor proc(
          std::make_unique<AudioParameterBool>(key, "", false));

      expect(proc.state.getParameterRange(key) ==
    ...
    beginTest("Float parameters retain their specified range");
    {
      ...
      TestAudioProcessor proc(
          std::make_unique<AudioParameterFloat>(key, "", range, 0.0f));
    ...
    beginTest("Int parameters retain their specified range");
    {
      ...
      TestAudioProcessor proc(
          std::make_unique<AudioParameterInt>(key, "", min, max, 0));

      expect(proc.state.getParameterRange(key) ==
             NormalisableRange<float>(float(min), float(max), 1.0f));
    }
    ...
    beginTest("Choice parameters retain their specified range");
    {
      ...
      TestAudioProcessor proc(
          std::make_unique<AudioParameterChoice>(key, "", choices, 0));

      expect(proc.state.getParameterRange(key) ==
             NormalisableRange<float>(0.0f, (float)(choices.size() - 1), 1.0f));
      expect(proc.state.getParameter(key)->getNumSteps() == choices.size());
    }

    beginTest("When the parameter value is changed, normal parameter values "
              "are updated");
    {
      TestAudioProcessor proc;
      const auto key = "id";
      ...
      auto param = proc.state.createAndAddParameter(std::make_unique<Parameter>(
          key, String(), NormalisableRange<float>(), initialValue));
    ...
    beginTest("When the parameter value is changed, custom parameter values "
              "are updated");
    {
      const auto key = "id";
