...
namespace juce {
    namespace dsp {
        ...
        void DelayLine<SampleType, InterpolationType>::setDelay(SampleType newDelayInSamples) {
            ...
            delay = jlimit<SampleType>(0.0f, upperLimit, newDelayInSamples);
            // delay = jlimit((SampleType)0, upperLimit, newDelayInSamples);
            //  Syl - if you're passing a negative delay here it's not going to work
            //  anyway, so the call to std::floor is totally redundant
            /* delayInt  = static_cast<int> (std::floor (delay)); */
            delayInt = static_cast<int>(delay);
            delayFrac = delay - static_cast<SampleType>(delayInt);
        ...
        template <typename SampleType, typename InterpolationType>
        void DelayLine<SampleType, InterpolationType>::setMaximumDelayInSamples(
        ...
        //==============================================================================
        template <typename SampleType, typename InterpolationType>
        void DelayLine<SampleType, InterpolationType>::pushSample(int channel,
                                                                  SampleType sample) {
            bufferData.setSample(channel, writePos[(size_t)channel], sample);
            writePos[(size_t)channel] =
                (writePos[(size_t)channel] + totalSize - 1) % totalSize;
        }
